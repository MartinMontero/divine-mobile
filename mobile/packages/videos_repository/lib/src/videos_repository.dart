...
class VideosRepository {
  ...
  Future<List<VideoEvent>> getNewVideos({
    ...
    if (!skipCache && until == null) {
      ...
      if (cached != null) {
        return _orderBySeenFreshness(cached.videos);
      }
    }
    ...
    if (_funnelcakeApiClient != null && _funnelcakeApiClient.isAvailable) {
      try {
        ...
        var hydrated = await _hydrateVideosWithBulkStats(mergedVideos);
        hydrated = await _orderBySeenFreshness(hydrated);
        if (until == null) {
    ...
    var hydrated = await _hydrateVideosWithBulkStats(videos);
    hydrated = await _orderBySeenFreshness(hydrated);
    if (until == null) {
      _inMemoryFeedCache?.set('latest', HomeFeedResult(videos: hydrated));
    }
